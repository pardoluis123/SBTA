...
class Processor:
    ...
    def replicate_array(self,replicates=None,name=None,datatype=None):
        ...
        for i in replicates:
            ...
            current_trajectory_array=current_processor.Average_Array()
            Final_Array[n-1]=current_trajectory_array
            Sum_Array[n-1]=np.sum(current_trajectory_array[1:,1:])
            Replicate_End=time()
            n+=1
    ...
    def Save_Full_Replicate_array(self,name=None,array=None):
        ...
        array:numpy.ndarray
            ...
            containing 2d arrays (each frame)
        """
        array,sum = array if array is not None else self.replicate_array()
        name = name if name is not None else self.name
        
        np.save(f"{self.data_directory}/{name}_Replicate_Array", array)
        return
