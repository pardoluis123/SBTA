...
class data_creator():
    def __init__(self,outname=None,Trajectory_or_Replicate_Path=None,Topology=None,filter_parameter=None,Residues_of_interest=None,slice_option=None):
        ...
        """
        Master class meant to execute data creation via the MDtraj school of thought

        Parameters
        ----------
        outname:string
            Outname for all the images that wll be outputed from the process

        Trajectory_or_Replicate_Path:string
            A string that leads to either the directory holding all of the  

        Topology:string
            A string leading to topology pertaining to this particular trajectory or its replicates
        
        Filter_Parameter:string
            A filter parameter by which to filter out only the trajectories inside of a given directory
        
        Residues_of_interest:list
            A list of numbers holding the 
        """
    ...
    def Full_array_routine(self,proc_object=None,outname=None,datatype=0):
        
        outname=outname if outname is not None else self.outname
        proc_object=proc_object if proc_object is not None else self.Processor_object_routine()

        proc_object.Save_Full_Replicate_array(name=outname)
...
if __name__ == "__main__":
    ...
    test_run = data_creator(outname=outname,Trajectory_or_Replicate_Path=Trajectory_or_Replicate_Path,Topology=Topology,filter_parameter=filter_parameter,Residues_of_interest=restrained_residue_list,slice_option=0)
